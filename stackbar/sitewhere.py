...
def survey(results, category_names):
    ...
    category_names : list of str
        The category labels.['#8797ae', '#afb9c8', '#d7dce4', '#eaebef', '#f7fbf4']
    """
    ...
    for i, (colname, color, new_color_1, new_color_2) in enumerate(zip(category_names, category_colors,category_colors_1, category_colors_2)):
        widths = data[:, i]
        starts = data_cum[:, i] - widths
        xcenters = starts + widths / 2
        colors = [color,new_color_1, new_color_2]
...
if __name__ == '__main__':
    ...
    fig.set_size_inches(11, 1.3)
    fig.tight_layout()
    plt.title('Sitewhere', fontsize = 16)

    plt.savefig('sitewhere-stack.eps', bbox_inches='tight')
    plt.show()
